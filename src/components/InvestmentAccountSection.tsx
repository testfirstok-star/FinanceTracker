...
export default function InvestmentAccountSection({ account }: { account: InvestmentAccount }) {
  ...
  return (
    <Collapsible
      title={
        <span>
          {account.name} <span className="ml-2 text-xs font-normal text-muted">net {formatMoney(totals.net)}</span>
        </span>
      }
      right={
        <button
    ...
    >
      <div className="mb-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
        ...
        <div className="rounded-md bg-panel-hover p-2">
          ...
          <div className="font-semibold text-accent-green">{formatMoney(totals.income)}</div>
        </div>
        <div className="rounded-md bg-panel-hover p-2">
          <div className="text-xs text-muted">Expenses</div>
          <div className="font-semibold text-accent-red">{formatMoney(totals.expenses)}</div>
        </div>
        <div className="rounded-md bg-panel-hover p-2">
          <div className="text-xs text-muted">Total net</div>
